...
- (void)setAllImgVContentFromImage:(NSArray *)imgList{
    ...
    if (imgList.count) {
        if ([imgList[0] isKindOfClass:urlCls]) {
            [vLift setImageWithURL:imgList[MRImgLocationLeft] placeholderImage:[UIImage imageNamed:@"picture_dafault_screen.jpg"]];
            [vCenter setImageWithURL:imgList[MRImgLocationCenter] placeholderImage:[UIImage imageNamed:@"picture_dafault_screen.jpg"]];
            [vRight setImageWithURL:imgList[MRImgLocationRight] placeholderImage:[UIImage imageNamed:@"picture_dafault_screen.jpg"]];
        }else{
...
{
    NSDictionary *dic=[NSDictionary dictionaryWithObject:[NSNumber numberWithLong:_curIndex] forKey:@"index"];
