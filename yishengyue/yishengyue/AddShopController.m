...
@interface AddShopController ()<UIImagePickerControllerDelegate,UINavigationControllerDelegate,UITextFieldDelegate,UITextViewDelegate>
{
...
- (IBAction)cancel:(id)sender {
    _warningWindow = [WKAlertView showAlertViewWithStyle:WKAlertViewStyleWaring title:@"取消发布商品" detail:nil canleButtonTitle:@"否" okButtonTitle:@"是" callBlock:^(MyWindowClick buttonIndex) {
...
- (IBAction)releaseShop:(id)sender {
    ...
    if (self.shopPriceTxt.text.doubleValue<=0.0) {
        [MessageFormat hintWithMessage:@"价格不能为0" inview:self.view completion:nil];
        return;
    }
    AppDelegate *myapp=[UIApplication sharedApplication].delegate;
    [self pleaseWaitInView:self.view];
    [[AFAppDotNetAPIClient sharedClient] POST:SaveMessage parameters:@{@"UserId":myapp.user.ID,@"Name":self.shopTitleTxt.text,@"Price":self.shopPriceTxt.text,@"Content":self.shopDetailTxt.text} constructingBodyWithBlock:^(id<AFMultipartFormData> formData) {
        int i=0;
        ...
        if (statuscode.intValue==0) {
            [MessageFormat hintWithMessage:@"添加成功" inview:self.view completion:^{
                [self dismissViewControllerAnimated:YES completion:nil];
...
//    if(offset <216.0)
//        self.contentView.frame = CGRectMake(0.0f, offset-216.0+64, self.contentView.frame.size.width, self.contentView.frame.size.height);
//    
