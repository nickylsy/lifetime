...
#define TVCELLREUSEIDENTIFIER @"tvbtncell"


@interface TVControlViewController ()
{
    NSString       * _currentXMLtag;
    NSString       * _currentXMLtagAttr;
    NSString       * _XMLAction;
    ControlButton  * _tempbtn;
    NSMutableArray * _qianarr;//前四个方形按钮
...
@implementation TVControlViewController
...
- (void)viewDidLoad {
    ...
    [self.collectionView registerClass:[RemotecontrolCollectionViewCell class] forCellWithReuseIdentifier:TVCELLREUSEIDENTIFIER];
...
-(void)menu:(UILongPressGestureRecognizer *)gesture
{
    if (gesture.state==UIGestureRecognizerStateBegan) {
        ...
        if (gesture.view.tag<=(TVINITBUTTONNUMBER_BOTTOM+TVINITBUTTONNUMBER_TOP)) {
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    RemotecontrolCollectionViewCell *cell=[collectionView dequeueReusableCellWithReuseIdentifier:TVCELLREUSEIDENTIFIER forIndexPath:indexPath];
    if (cell) {
        ...
        switch (indexPath.item) {
                ...
                
            {
                ...
                cell.contentLab.text=@"AV/TV";
                break;
            }
            case 2:
            {
                cell.contentImg.image=[UIImage imageNamed:@"closevoice.png"];
                cell.contentLab.text=nil;
                break;
            }
            case 3:
            {
                cell.contentImg.image=nil;
                cell.contentLab.text=@"返回";
                break;
            }
                
            default:
            {
                cell.contentLab.text=btn.name;
                cell.contentImg.image=nil;
                break;
            }
        }
...
//        case 0:
//        {
//            Device *dev=[ControlButton buttonWithName:@"NULL" ID:[NSString stringWithFormat:@"%@00001",_currentdevice.ID]];
//#warning 按钮名字
//            if (![_qianarr containsObject:dev]) {
//            
//            return;}
//        }
//        case 1:
//            
//        {
//            if (![_qianarr containsObject:[ControlButton buttonWithName:@"av/tv" ID:[NSString stringWithFormat:@"%@00002",_currentdevice.ID]]]) {
//            
//            return;}
//        }
//        case 2:
//        {
//            if (![_qianarr containsObject:[ControlButton buttonWithName:@"audio" ID:[NSString stringWithFormat:@"%@00003",_currentdevice.ID]]]) {
//            
//            return;}
//        }
//        case 3:
//        {
//            if (![_qianarr containsObject:[ControlButton buttonWithName:@"back" ID:[NSString stringWithFormat:@"%@00004",_currentdevice.ID]]]) {
//            
//            return;}
//        }
//            
...
- (IBAction)control:(UIButton *)sender {
    NSString *controlstr=[MessageFormat controlWithboxID:_myapp.currentbox.ID btnID:[NSString stringWithFormat:@"%@%05li",_currentdevice.ID,sender.tag]];
    [MessageFormat sendMessage:controlstr socketDelegate:self tag:1];
}
...
-(void)parser:(NSXMLParser *)parser foundCharacters:(NSString *)string
{
    ...
    if ([_XMLAction isEqualToString:@"allbutton"]) {
        if ([_currentXMLtag isEqualToString:@"btnid"]) {
            ...
            if ([string substringFromIndex:5].intValue>(TVINITBUTTONNUMBER_TOP+TVINITBUTTONNUMBER_BOTTOM)) {
                [_zhongarr addObject:btn];
            }else if ([string substringFromIndex:5].intValue>TVINITBUTTONNUMBER_TOP)
            {
...
-(void)parserDidEndDocument:(NSXMLParser *)parser
{
    if ([_XMLAction isEqualToString:@"allbutton"]) {
        ...
        ControlButton *btnav=[ControlButton buttonWithName:@"av/tv" ID:[NSString stringWithFormat:@"%@00002",_currentdevice.ID]];
        ControlButton *btnaudio=[ControlButton buttonWithName:@"audio" ID:[NSString stringWithFormat:@"%@00003",_currentdevice.ID]];
        ControlButton *btnback=[ControlButton buttonWithName:@"back" ID:[NSString stringWithFormat:@"%@00004",_currentdevice.ID]];
        [_collectionviewarr addObject:btnclose];
        [_collectionviewarr addObject:btnav];
        [_collectionviewarr addObject:btnaudio];
        [_collectionviewarr addObject:btnback];
