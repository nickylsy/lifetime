//
//  AircleanerControlViewController.m
//  AirboxConfig
//
...
#define AIRCLEANERINITBUTTONNUMBER_TOP 2
#define AIRCLEANERINITBUTTONNUMBER_BOTTOM 3
#define AIRCLEANERCELLREUSEIDENTIFIER @"aircleanerbtncell"

@interface AircleanerControlViewController ()
{
    NSString       * _currentXMLtag;
    NSString       * _currentXMLtagAttr;
    NSString       * _XMLAction;
    ControlButton  * _tempbtn;
    NSMutableArray * _qianarr;
    NSMutableArray * _zhongarr;
    NSMutableArray * _houarr;
...
@implementation AircleanerControlViewController
...
- (void)viewDidLoad {
    ...
    [self.collectionView registerClass:[RemotecontrolCollectionViewCell class] forCellWithReuseIdentifier:AIRCLEANERCELLREUSEIDENTIFIER];
...
-(void)menu:(UILongPressGestureRecognizer *)gesture
{
    if (gesture.state==UIGestureRecognizerStateBegan) {
        ...
        if (gesture.view.tag<=(AIRCLEANERINITBUTTONNUMBER_BOTTOM+AIRCLEANERINITBUTTONNUMBER_TOP)) {
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    RemotecontrolCollectionViewCell *cell=[collectionView dequeueReusableCellWithReuseIdentifier:AIRCLEANERCELLREUSEIDENTIFIER forIndexPath:indexPath];
    if (cell) {
...
-(void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath
{
    switch (indexPath.item) {
        case 0:
        {
            if (![_qianarr containsObject:[ControlButton buttonWithName:@"close" ID:[NSString stringWithFormat:@"%@00001",_currentdevice.ID]]]) {
                return;
            }
        ...
        {
            if (![_qianarr containsObject:[ControlButton buttonWithName:@"mode" ID:[NSString stringWithFormat:@"%@00002",_currentdevice.ID]]]) {
                return;
            }
        }
        default:
...
-(void)parser:(NSXMLParser *)parser foundCharacters:(NSString *)string
{
    ...
    if ([_XMLAction isEqualToString:@"allbutton"]) {
        if ([_currentXMLtag isEqualToString:@"btnid"]) {
            ...
            if ([string substringFromIndex:5].intValue>(AIRCLEANERINITBUTTONNUMBER_TOP+AIRCLEANERINITBUTTONNUMBER_BOTTOM)) {
                [_zhongarr addObject:btn];
            }else if ([string substringFromIndex:5].intValue>AIRCLEANERINITBUTTONNUMBER_TOP)
            {
    ...
    if ([_XMLAction isEqualToString:@"maxid"]) {
        if ([_currentXMLtag isEqualToString:@"btnid"]) {
            if ([string substringFromIndex:5].intValue>(AIRCLEANERINITBUTTONNUMBER_TOP+AIRCLEANERINITBUTTONNUMBER_BOTTOM)) {
                ...
                _tempbtn.ID=[NSString stringWithFormat:@"%@%05i",_currentdevice.ID,TVINITBUTTONNUMBER_BOTTOM+TVINITBUTTONNUMBER_TOP];
            }
...
-(void)parserDidEndDocument:(NSXMLParser *)parser
{
    if ([_XMLAction isEqualToString:@"allbutton"]) {
        ...
        ControlButton *btnmode=[ControlButton buttonWithName:@"mode" ID:[NSString stringWithFormat:@"%@00002",_currentdevice.ID]];
        [_collectionviewarr addObject:btnclose];
        [_collectionviewarr addObject:btnmode];
