...
-(void)setdefaultSelectedWithProvince:(NSString *)province City:(NSString *)city Area:(NSString *)area
{
    int p=0,c=0,a=0;
    for (int i=0; i<provinces.count; i++) {
        NSString *s=provinces[i];
        if ([s isEqualToString:province]) {
            p=i;
            break;
        }
    }
    ...
    for (int i=0; i<areas.count; i++) {
        NSString *s=areas[i];
        if ([s isEqualToString:area]) {
            a=i;
            break;
        }
    }
    
    [self.locatePicker selectRow:p inComponent:0 animated:NO];
    [self.locatePicker selectRow:c inComponent:1 animated:NO];
    [self.locatePicker selectRow:a inComponent:2 animated:NO];
}
@end
