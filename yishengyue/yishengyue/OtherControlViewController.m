//
//  OtherControlViewController.m
//  AirboxConfig
//
//  Created by Xtoucher08 on 15/5/12.
...
#define OTHERCELLREUSEIDENTIFIER @"otherbtncell"

@interface OtherControlViewController ()
{
...
@implementation OtherControlViewController
...
- (void)viewDidLoad {
    ...
    [self.collectionView registerClass:[RemotecontrolCollectionViewCell class] forCellWithReuseIdentifier:OTHERCELLREUSEIDENTIFIER];
...
-(void)menu:(UILongPressGestureRecognizer *)gesture
{
    if (gesture.state==UIGestureRecognizerStateBegan) {
            ...
            NSArray *titles=[NSArray arrayWithObjects:@"学习",@"重命名",@"删除", nil];
            NSArray *styles=[NSArray arrayWithObjects:YUDefaultStyle,YUDefaultStyle,YUDangerStyle, nil];
...
- (NSInteger)collectionView:(UICollectionView *)collectionView numberOfItemsInSection:(NSInteger)section {
    return _currentdevice.btns.count;
}

- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    RemotecontrolCollectionViewCell *cell=[collectionView dequeueReusableCellWithReuseIdentifier:OTHERCELLREUSEIDENTIFIER forIndexPath:indexPath];
    if (cell) {
        
        ControlButton *btn=_currentdevice.btns[indexPath.item];
...
-(void)parser:(NSXMLParser *)parser didStartElement:(NSString *)elementName namespaceURI:(NSString *)namespaceURI qualifiedName:(NSString *)qName attributes:(NSDictionary *)attributeDict
{
    ...
    if ([elementName isEqualToString:@"para"]) {
        ...
        if ([_XMLAction isEqualToString:@"allbutton"]) {
            ...
            [_currentdevice.btns removeAllObjects];
        }
    }
...
-(void)parser:(NSXMLParser *)parser foundCharacters:(NSString *)string
{
    ...
    if ([_XMLAction isEqualToString:@"allbutton"]) {
        if ([_currentXMLtag isEqualToString:@"btnid"]) {
            ...
            [_currentdevice.btns addObject:btn];
        }
    }
