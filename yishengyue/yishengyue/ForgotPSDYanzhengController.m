...
#import "ForgotPSDYanzhengController.h"
#import "Mydefine.h"
...
@interface ForgotPSDYanzhengController ()<UITextFieldDelegate>
{
    NSTimer *_timer;
    int _shengyushijian;
...
@implementation ForgotPSDYanzhengController
...
- (IBAction)next:(UIButton *)sender {
    ...
    [[AFAppDotNetAPIClient sharedClient] POST:ValidateMessage.URLEncodedString parameters:@{@"Tel":_tempPhoneNum,@"Message":self.yanzhengmaTxt.text} success:^(NSURLSessionDataTask *task, id responseObject) {
        ...
        if (statuscode.intValue==0) {
            ...
            ResetPasswordController *resetcontroller=[autolayoutstoryboard instantiateViewControllerWithIdentifier:@"resetpasswordcontroller"];
            resetcontroller.phonenumber=self.phoneTxt.text;
...
- (IBAction)getyanzheng:(UIButton *)sender {
    ...
    [[AFAppDotNetAPIClient sharedClient] POST:EditMessage.URLEncodedString parameters:@{@"Tel":_tempPhoneNum} success:^(NSURLSessionDataTask *task, id responseObject) {
        NSNumber *statuscode=responseObject[@"code"];
        if (statuscode.intValue!=0) {
            [MessageFormat hintWithMessage:responseObject[@"message"] inview:self.view completion:nil];
        }
